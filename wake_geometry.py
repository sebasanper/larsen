...
def wake_speed(U0, ct, A, x, r, c1, x0):
    return U0 * (1.0 - ((ct * A * (x + x0) ** (- 2.0)) ** (1.0 / 3.0)) / 9.0 * (r ** (3.0 / 2.0) * (3.0 * c1 ** 2.0 * ct * A * (x + x0)) ** (- 1.0 / 2.0) - (35.0 / 2.0 / pi) ** (3.0 / 10.0) * (3.0 * c1 ** 2.0) ** (- 1.0 / 5.0)) ** 2.0)


def wake_deficit(U0, ct, A, x, r, c1, x0):
    return 1.0 - wake_speed(U0, ct, A, x, r, c1, x0) / U0
...
if __name__ == '__main__':
    ...
    H = 100.0  # Hub height
    ia = 0.08  # Ambient turbulence intensity
    ...
    for x in range(1, 560):
        print wake_deficit(8.5, 0.79, A, x, 0.0, c1)